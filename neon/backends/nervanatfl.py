...
# ******************************************************************************
"""
Tensorflow based backend interface and tensor data structure. Our implementation
...
from neon.backends.layer_tfl import ConvLayer, DeconvLayer, PoolLayer
...
class TFLTensor(Tensor):

    """
    The n-dimensional array data structure that resides in host memory,
    and is meant to be manipulated on the device.  wrapped `numpy.ndarray` tensor.
    ...
    def __init__(self,
        ...
        super(TFLTensor, self).__init__(backend, shape, dtype, name,
                                        persist_values)
    ...
    def __str__(self):
        ...
        return ("TFLTensor(base 0x%x) name:%s shape:%s dtype:%s strides:%s"
    ...
    def __setitem__(self, key, value):
        ...
        Arguments:
            ...
            value (numeric array, TFLTensor): values to be assigned to the
    ...
    def __getitem__(self, key):
        ...
        Tensors, TFL Tensors remove the axis that has size 1 unless it needs to
        maintain 2D.
        ...
        Returns:
            TFLTensor: view of self corresponding to the subset items.
        ...
        # xxx - fix this comment for tensorflow, what does this mean?
    ...
    def _assign(self, value):
        """
        Assign an input value to the TFL tensor. The NervanaTFL does clipping
        for int and uint types, when overflow happens

        Arguments:
            value (TFLTensor, OpTreeNode, numeric): the value to be assigned.

        """
        if isinstance(value, (TFLTensor, OpTreeNode)):
    ...
    def set(self, value):
        """
        Wrap the value into NervanaTFL tensor.
    ...
    def fill(self, value):
        """
        Assign specified value to each element of this TFLTensor.

        Arguments:
            value (numeric): The value to be assigned to each element.

        Return:
            TFLTensor: updated view of the data.
        """
...
class NervanaTFL(Backend):
    ...
    Attributes:
        ...
        tensor_cls: underlying Tensor type. For tensorflow backend, it will be TFL tensor
    ...
    def __init__(self,
        ...
        super(NervanaTFL, self).__init__(rng_seed, default_dtype, compat_mode=compat_mode)
        ...
        try:
            if not any(x in str(np.__config__.blas_opt_info['libraries']).lower()
                ...
                logger.warn("No accelerated BLAS libraries found, "
                            "performance may suffer.  Consider installing "
                            "one of openblas, Atlas, MKL, or vecLib")
        except (AttributeError, KeyError):
            if sys.platform != 'darwin':
                logger.warn("Problems inferring BLAS info, performance may "
                            "be suboptimal")

        self.device_type = 0
        self.device_id = 0
        self.tensor_cls = TFLTensor

        logger.info("Initialized NervanaTFL")
    ...
    def execute(self, optree, numpy_call_dict=numpy_call_dict_tfl):
        """
    ...
    def empty(self, shape, dtype=None, name=None, persist_values=True,
        ...
        """
        Instantiate a new instance of the TFLTensor class without initializing
        ...
        Returns:
            TFLTensor: newly created data structure reference
        """
    ...
    def array(self, ary, dtype=None, name=None, persist_values=True,
              parallel=False, distributed=False):
        """
        Instantiate a new instance of the TFLTensor class setting each element
    ...
    def fprop_conv(self, layer, I, F, O,
        ...
        Arguments:
            ...
            I (TFLTensor): inputs
            F (TFLTensor): the weights (filters)
            O (TFLTensor): outputs
    ...
    def bprop_conv(self, layer, F, E, grad_I,
        ...
        Arguments:
            ...
            F (TFLTensor): the weights (filters)
            E (TFLTensor): errors
            grad_I (TFLTensor): gradient to inputs (output delta)
    ...
    def update_conv(self, layer, I, E, U, alpha=1.0, beta=0.0, grad_bias=None, layer_op=None):
        ...
        Arguments:
            layer: the conv layer as a parameter object
            I (TFLTensor): the inputs
            E (TFLTensor): the errors
            U (TFLTensor): the updates
    ...
    def synchronize_mark(self, marker):
        ...
        Arguments:
            marker (time mark): timing mark generated by init_mark()
        """
        # No-op on cpu, xxx - for tensorflow?
        return
